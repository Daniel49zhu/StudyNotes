...
- 第4章 模块化React和Redux应用 
   ...
   从架构出发，当开始一个新的应用的时候，有几件事一定要考虑清楚
   1. 代码问价你的组织结构
   2. 确定模块的边界
   3. Store的状态树的设计
   
   这三件事是构建一个应用的基础。本章我们将建造一个TodoList应用。
   
   代码的组织形式无外乎按照角色组织和按照功能组织两种形式，React应用会依然按照“按功能组织”的
   方式。
   
   Store的状态树设计，直接决定了要写哪些reducer，还有action怎么写，
   设计状态树主要有如下几个原则
   1. 一个模块控制一个状态节点
   2. 避免冗余数据
   3. 树形结构扁平化
